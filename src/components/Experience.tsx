...
const Experience: React.FC = () => {
  ...
  return <section id="experience" className="section-padding py-28">
        ...
        <div className="max-w-4xl mx-auto">
          <div className="flex flex-col md:flex-row gap-4 reveal">
            <div className="md:w-1/4 mb-4 md:mb-0 flex md:flex-col overflow-x-auto md:overflow-visible scrollbar-thin">
              {experienceData.map((item, index) => <button key={item.id} onClick={() => setActiveTab(index)} className={cn("py-3 px-4 text-left border-b-2 md:border-b-0 md:border-l-2 font-mono text-sm whitespace-nowrap md:whitespace-normal", activeTab === index ? "border-tech-light-blue text-tech-light-blue bg-tech-navy/30" : "border-tech-light-navy text-tech-light-slate hover:bg-tech-navy/20 hover:text-tech-light-blue")}>
                  {item.company}
                </button>)}
            </div>
            ...
            <div className="md:w-3/4 bg-tech-navy/20 p-6 rounded-md glass animate-fade-in">
              ...
              <ul className="space-y-2 mb-4">
                {experienceData[activeTab].description.map((item, index) => <li key={index} className="flex items-start">
              ...
              <div className="flex flex-wrap gap-2 mt-4">
                {experienceData[activeTab].technologies.map((tech, index) => <span key={index} className="text-xs font-mono bg-tech-navy py-1 px-2 rounded text-tech-light-blue">
                    {tech}
                  </span>)}
              </div>
            </div>
