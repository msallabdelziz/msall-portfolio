...
const Projects: React.FC = () => {
  ...
  useEffect(() => {
    if (filter === 'all') {
      ...
      setFilteredProjects(projectsData.filter(project => Array.isArray(project.category) ? project.category.includes(filter) : project.category === filter));
    }
  }, [filter]);
  useEffect(() => {
  ...
  const renderDescription = (project: Project) => {
    ...
    if (project.description.length <= maxLength || isExpanded) {
      return <>
          ...
          {project.description.length > maxLength && <button onClick={() => toggleDescription(project.id)} className="text-tech-light-blue flex items-center text-sm mb-4 hover:text-tech-lightest-slate transition-colors" aria-label={isExpanded ? "Voir moins" : "Voir plus"}>
              {isExpanded ? "Voir moins" : "Voir plus..."}
              {isExpanded ? <ChevronUp size={16} className="ml-1" /> : <ChevronDown size={16} className="ml-1" />}
            </button>}
        </>;
    } else {
      return <p className="text-tech-light-slate mb-4">
          {project.description.substring(0, maxLength)}... 
          <button onClick={() => toggleDescription(project.id)} className="text-tech-light-blue inline-flex items-center text-sm ml-2 hover:text-tech-lightest-slate transition-colors" aria-label="Voir plus">
            Voir plus...
  ...
  return <section id="projects" className="section-padding py-28 bg-tech-dark-blue/30">
      <div className="container mx-auto">
        ...
        <div className="flex justify-center mb-10 reveal">
          <div className="inline-flex p-1 space-x-1 bg-tech-navy rounded-lg">
            {['all', 'data', 'web', 'mobile'].map(category => <button key={category} onClick={() => setFilter(category)} className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${filter === category ? 'bg-tech-light-blue text-tech-dark-blue' : 'text-tech-light-slate hover:text-tech-lightest-slate'}`}>
                {category === 'all' ? 'All' : category === 'data' ? 'IA / DATA' : category.charAt(0).toUpperCase() + category.slice(1)}
              </button>)}
          </div>
        </div>
        
        <div className="grid md:grid-cols-2 gap-8">
          {filteredProjects.map((project, index) => <div key={project.id} className="group glass hover:shadow-xl transition-all duration-300 overflow-hidden reveal" style={{
          transitionDelay: `${index * 100}ms`
              ...
              <div className="p-6">
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {project.technologies.map((tech, i) => <span key={i} className="text-xs font-mono bg-tech-navy py-1 px-2 rounded text-tech-light-blue">
                      {tech}
                    </span>)}
                </div>
                
                <div className="flex items-center space-x-4">
                  {project.github && <a href={project.github} target="_blank" rel="noopener noreferrer" className="text-tech-light-slate hover:text-tech-light-blue transition-colors" aria-label="Voir le code sur GitHub">
                      <Github size={20} />
                    </a>}
                  {project.demo && <a href={project.demo} target="_blank" rel="noopener noreferrer" className="text-tech-light-slate hover:text-tech-light-blue transition-colors" aria-label="Voir la démo">
